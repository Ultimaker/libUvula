...
SKILL_TRIGGERS = {
    'cmake': (
        ...
        'Target-centric CMake, presets, and CTest wiring — avoid reinventing build logic or reaching for directory-scoped commands',
    ),
    'conan-2': (
        ['**/conanfile.py', '**/conanfile.txt', '**/conandata.yml'],
        'Dependency graph, profiles, cross-compilation and packaging are Conan 2 concerns',
    ),
    'cpp-pro': (
        ['**/*.cpp', '**/*.hpp', '**/*.cc', '**/*.cxx', '**/*.h'],
        'Modern C++20/23 implementation work: templates, zero-overhead abstractions, and the idioms mined into the C++ architecture rule',
    ),
    'ultimaker-neoprep-development': (
        ['UvulaJS/**', '**/UvulaJS/**', '**/*UvulaJS*'],
        'Neoprep WebAssembly bindings, Three.js viewport stroke projection, and Emscripten runtime contract',
    ),
    'ultimaker-cura-development': (
        ['pyUvula/**', '**/pyUvula/**', '**/*pyUvula*'],
        'Cura native C++ core and Python bindings (pyUvula) packaging and integration',
    ),
    'ultimaker-cloud-development': (
        ['UvulaJS/**', '**/UvulaJS/**'],
        'Digital Factory cloud web slicer and preparation service architecture',
    ),
}
...
def main():
    ...
    if not hits:
        return
    msg_lines = [
        "Relevant UltiCortex skills for the files you are about to change — load these BEFORE writing the code:" if pre_edit else
        "Relevant UltiCortex skills for the files you are changing:"
    ]
    for skill, (reason, paths) in sorted(hits.items()):
        sample = ", ".join(paths[:3]) + (" ..." if len(paths) > 3 else "")
        msg_lines.append("  - {}: {}".format(skill, reason))
        msg_lines.append("      triggered by: {}".format(sample))
        msg_lines.append("      gh skill install Ultimaker/UltiCortex {}".format(skill))
    msg = "\n".join(msg_lines)
