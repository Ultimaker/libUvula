...
Description: Finds the cross-repository surfaces of `libUvula` that no file-extension detector can see, and files them as `ecosystem_contract` observations.
...
You are investigating what `libUvula` promises to, and expects from, the
