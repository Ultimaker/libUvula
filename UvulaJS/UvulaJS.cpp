...
UnwrapResult unwrap(const Float32Array& vertices_js, const Uint32Array& indices_js)
{
    ...
    if (vertices_length == 0 || indices_length == 0 || vertices_length % 3 != 0 || indices_length % 3 != 0)
    {
        return UnwrapResult{ .uvCoordinates = Float32Array{ emscripten::val::global("Float32Array").new_(0) }, .textureWidth = 0, .textureHeight = 0 };
    }
    ...
    // Perform unwrapping with exception safety
    try
    {
    ...
    uint32_t face_id)
{
    ...
    auto matrix_length = camera_projection_matrix_js["length"].as<int>();

    if (stroke_length < 6 || stroke_length % 2 != 0 || matrix_length < 16)
    {
        return PolygonArray{ emscripten::val::array() };
    }
    ...
    // Call the projection function with exception safety
